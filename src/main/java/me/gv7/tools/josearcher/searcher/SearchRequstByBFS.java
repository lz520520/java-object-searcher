...
public class SearchRequstByBFS {
    ...
    public SearchRequstByBFS(Object target, Keyword key){
        //打开调试模式,会生成log日志
        this.is_debug = true;
        //挖掘深度为10
        this.max_search_depth = 10;
        //设置报告保存位置
        this.report_save_path = "./tmp/";
    ...
    public void initSavePath(){
        ...
        if(report_save_path == null){
            ...
            File file = new File(this.report_save_path);
            file.mkdirs();
            ...
            if (filed_object == null || CheckUtil.isSysType(filed_object) || MatchUtil.isInBlacklist(filed_name,filed_object,this.blacklists)){
               ...
               if (filed_object != null) {
                    ...
                    if (log_chain != null && !log_chain.equals("")) {
                        new_log_chain = String.format("%s \n%s ---> %s%s = {%s}", log_chain, getBlank(current_depth),FINAL_FLAG, filed_name, filed_object.getClass().getName());
                    } else {
                        new_log_chain = String.format(FINAL_FLAG+ "%s = {%s}", "TargetObject", filed_object.getClass().getName());
                    }

                    // 搜索操作
                    if (MatchUtil.matchObject(filed_name, filed_object, keys)) {
                        write2log(result_file, new_log_chain + "\n");
                        write2log(result_file, "idea_express: " + genExpress2(new_log_chain) + "\n\n\n");                    }
                    if (is_debug) {
